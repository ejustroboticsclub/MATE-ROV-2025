...
void setup() {
  ...
  while (Serial.available() == 0){
      ...
      // Read command from serial (expecting 'CALIBRATE')
  String command = Serial.readStringUntil('\n');
  ...
  // Start with default calibration
  // calibrateSensor();
}

void loop() {
  // Check if the serial input has a new command
  if (Serial.available()) {
    String command = Serial.readStringUntil('\n');
  ...
  if (IMU.accelerationAvailable() && IMU.gyroscopeAvailable()) {
    ...
    // Convert gyroscope readings from degrees to radians before passing to the quaternion function
    double Gx_rad = Gx * DEG_TO_RAD_;
    double Gy_rad = Gy * DEG_TO_RAD_;
    double Gz_rad = Gz * DEG_TO_RAD_;
    
    CalculationQuaternion(Gx_rad - gyroOffsetX, Gy_rad - gyroOffsetY, Gz_rad - gyroOffsetZ, 
                          (Ax) * 9.80665 - accelOffsetX, (Ay) * 9.80665 - accelOffsetY, (Az) * 9.80665 - accelOffsetZ);

    // EulerAngles angles = quaternionToEulerDegrees(q0_, q1_, q2_, q3_);
    ...
    Serial.print(q3_);Serial.print(",");
    Serial.print(Gz_rad - gyroOffsetZ);Serial.println();
  }
