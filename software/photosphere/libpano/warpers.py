...
def interpolate_image(image, mask):
    ...
    _, blurred_mask = cv.threshold(
        cv.cvtColor(blurred_mask, cv.COLOR_BGR2GRAY), 1, 255, cv.THRESH_BINARY
    )
    ...
    #
    int_im = cv.resize(
        int_im, (final_mask.shape[1], final_mask.shape[0]), 0, 0, cv.INTER_LINEAR_EXACT
    )
...
def spherical_warp(image, pitch, metrics):
    ...
    fov = np.array([metrics.AOV_h / pi2, metrics.AOV_v / math.pi], dtype=float)

    # generate image map
    xx, yy = np.meshgrid(
        np.linspace(0, 1, image_width), np.linspace(0, 1, image_height)
    )
    ...
    image_map = (
        (image_map * 2 - 1)
        * np.array([math.pi, pi_2])
        * (np.ones_like(image_map) * fov)
    )
    ...
    lat = np.arcsin(
        cos_c * np.sin(center_point[1]) + (y * sin_c * np.cos(center_point[1])) / rou
    )
    lon = center_point[0] + np.arctan2(
        x * sin_c,
        rou * np.cos(center_point[1]) * cos_c - y * np.sin(center_point[1]) * sin_c,
    )

    lat = (lat / pi_2 + 1.0) * 0.5
    lon = (lon / math.pi + 1.0) * 0.5
    ...
    warped = np.zeros((pano_height, pano_width, pano_channel), np.float64)
    ...
    # mask image process
    mask_img = np.ones_like(flat_img, np.float64) * 255
    mask_warped = np.zeros_like(warped, np.float64)
    mask_warped[flat_idx] = mask_img

    # reshape into their original shape
    warped = np.reshape(warped, [pano_height, pano_width, pano_channel]).astype(
        np.uint8
    )
    mask_warped = np.reshape(
        mask_warped, [pano_height, pano_width, pano_channel]
    ).astype(np.uint8)
...
def cylindrical_warp_with_focal(img, focal_length):
    ...
    for y in range(-int(height / 2), int(height / 2)):
        for x in range(-int(width / 2), int(width / 2)):
            ...
            if (
                (cylinder_x >= 0)
                and (cylinder_x < width)
                and (cylinder_y >= 0)
                and (cylinder_y < height)
            ):
                cylinder_proj[cylinder_y][cylinder_x] = img[y + int(height / 2)][
                    x + int(width / 2)
                ]
    ...
    _, thresh = cv.threshold(
        cv.cvtColor(cylinder_proj, cv.COLOR_BGR2GRAY), 1, 255, cv.THRESH_BINARY
    )
...
def cylindrical_warp_with_k(img, k):
    ...
    img = cv.remap(
        img,
        b[:, :, 0].astype(np.float6432),
        b[:, :, 1].astype(np.float6432),
        cv.INTER_AREA,
...
def render_image_to_canvas(canvas, image, pitch, yaw, metrics):
    ...
    fov = np.array([metrics.AOV_h / pi2, metrics.AOV_v / math.pi], np.float64)
    ...
    temp_canvas = np.reshape(
        temp_canvas, [pano_height, pano_width, pano_channel]
    ).astype(np.uint8)
    ...
    # reshape into their original shape
    canvas = np.reshape(canvas, [pano_height, pano_width, pano_channel]).astype(
        np.uint8
    )
