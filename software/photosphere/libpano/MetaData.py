...
class MetaData:
    ...
    def get_refined_dataframe(self):
        ...
        meta_pd = pd.DataFrame(
            {
                ...
                "pitch": [
                    item["sensors"]["roll_pitch_yaw"]["pitch"] for item in pictures
                ],
                "roll": [
                    item["sensors"]["roll_pitch_yaw"]["roll"] for item in pictures
                ],
        ...
        sorted_rings = (
            meta_pd.groupby(by="ring")["pitch"].agg(np.mean).sort_values().index
        )
        ...
        for row, ring in enumerate(sorted_rings):
            ...
            df = pd.concat([df, sub_df[df.columns]], ignore_index=False)
...
class PanoMetrics:
    ...
    def __init__(
        ...
        uri = meta_data[
            (meta_data.row == self.N_v // 2) & (meta_data.col == self.N_h // 2)
        ]["uri"].values[0]
    ...
    def __str__(self):
        ...
        string += "\tPixels per mm: {:.2f} x {:.2f} px\n".format(
            self.FW / self.sensor_width, self.FH / self.sensor_height
        )
        ...
        string += "\tInterval Angle: {}︒ x {}︒\n".format(
            utils.radian2degree(self.FIA_h), utils.radian2degree(self.FIA_v)
        )
        string += "\tAoV: {:.4f}︒ x {:.4f}︒\n".format(
            utils.radian2degree(self.AOV_h), utils.radian2degree(self.AOV_v)
        )
    ...
    def get_non_overlapping_width(image_file1, image_file2):
        ...
        source_points = np.float32(
            [key_points1[m.queryIdx].pt for m in good_matches]
        ).reshape(-1, 1, 2)
        destination_points = np.float32(
            [key_points1[m.trainIdx].pt for m in good_matches]
        ).reshape(-1, 1, 2)
        ...
        non_overlapping_widths = [
            key_points1[m.queryIdx].pt[0] - key_points2[m.trainIdx].pt[0]
            for m in matches
        ]
    ...
    def get_non_overlapping_height(image_file1, image_file2):
        ...
        destination_points = np.float32(
            [key_points2[m.trainIdx].pt for m in good_matches]
        ).reshape(-1, 1, 2)
        ...
        # get NOH
        non_overlapping_widths = [
            key_points1[m.queryIdx].pt[1] - key_points2[m.trainIdx].pt[1]
            for m in matches
        ]
    ...
    def get_aov_h(self):
        ...
        fn1 = self.meta_data[
            (self.meta_data.row == best_row) & (self.meta_data.col == best_col)
        ]["uri"].values[0]
        fn2 = self.meta_data[
            (self.meta_data.row == best_row) & (self.meta_data.col == best_col + 1)
        ]["uri"].values[0]
    ...
    def get_aov_v(self):
        ...
        fn2 = self.meta_data[
            (self.meta_data.row == best_row + 1) & (self.meta_data.col == best_col)
        ]["uri"].values[0]
